...
public class VendorServiceImpl implements VendorService { 
    ...
    @Override public Vendor createVendor(Vendor vendor) { 
        if (repository.findByVendorName(vendor.getVendorName()).isPresent()) throw new 
IllegalArgumentException("Vendor name exists"); 
        if (vendor.getContactEmail() == null || !vendor.getContactEmail().contains("@")) throw new 
IllegalArgumentException("Invalid email"); 
        return repository.save(vendor); 
    } 
    @Override public List<Vendor> getAllVendors() { return repository.findAll(); } 
    @Override public Vendor getVendor(Long id) { return repository.findById(id).orElseThrow(() -> 
